...
class TransKun(torch.nn.Module):
    ...
    def transcribeFrames(self,framesBatch, forcedStartPos= None, velocityCriteron = "hamming", onsetBound = None, candidateNotes=[]):
        ...
        if onsetBound is not None:
            path = [[e for e in segment if e[0] < onsetBound] for segment in path]

        lastP = [segment[-1][1] if segment else 0 for segment in path]
        ...
        nIntervalsAll = sum([len(segment) for segment in path])
        if nIntervalsAll == 0:
            return [[] for _ in range(nBatch)], lastP

        intervalsBatch = [path[i * nSymbols: (i + 1) * nSymbols] for i in range(nBatch)]
        ...
        for idx, candidateNotesForSegment in enumerate(candidateNotes):
        # for idx in range(nBatch):
            if idx < len(intervalsBatch):
                curIntervals = intervalsBatch[idx] if idx < len(intervalsBatch) else []
                print(idx)
                notesBatch = []
                #print(range(nBatch))
                ...
                for j, eventType in enumerate(candidateNotesForSegment):
                    print(j, eventType)
                    if eventType in self.targetMIDIPitch:
                        j = self.targetMIDIPitch.index(eventType)
                    else:
                        j = self.targetMIDIPitch.index(2)
    ...
    def transcribe(self, x, stepInSecond = 1, segmentSizeInSecond = 1, discardSecondHalf=False, candidateNotes=[]):
        ...
        startPos = [math.floor(padTimeBegin * self.fs / self.hopSize)] * len(self.targetMIDIPitch)
        ...
        for i in range(0, nSample, stepSize):
            ...
            # Calculate segment index for the candidate notes
            segmentIndex = i // (self.fs * stepInSecond)
            if segmentIndex < len(candidateNotes):
                segmentCandidateNotes = candidateNotes[segmentIndex]
            else:
                segmentCandidateNotes = []

            curEvents, lastP = self.transcribeFrames(curFrames.unsqueeze(0), forcedStartPos = startPos, velocityCriteron = "hamming", onsetBound= onsetBound, candidateNotes=segmentCandidateNotes)
            curEvents = curEvents[0]


            #startPos = []
            startPos = [max(p - int(stepSize / self.hopSize), 0) for p in lastP]
            for k in lastP:
